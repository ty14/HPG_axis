...
y1a <- readRDS("males/clean_data/limma_eFDR_ARC.RDS") %>%
  distinct(.) %>% 
 filter(abs(logFC) >= my_logFC_threshold)

y2a <- readRDS("males/clean_data/limma_eFDR_VMN.RDS") %>% 
  distinct(.) %>% 
  filter(abs(logFC) >= my_logFC_threshold)

y3a <- readRDS("males/clean_data/limma_eFDR_testis.RDS")%>% 
  distinct(.) %>% 
  filter(abs(logFC) >= my_logFC_threshold)

y4a <- readRDS("males/clean_data/limma_eFDR_PIT.RDS")%>% 
  distinct(.) %>% 
  filter(abs(logFC) >= my_logFC_threshold)
...
write.csv(top10go_all, "males/clean_data/GOTerms_males_limma_allgenes_LogFC0.2.csv", row.names = F)
